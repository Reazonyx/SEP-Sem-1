...
public class VIAClub extends Application
{
   ...
   /**
    * Creates a method called initialize, cause SceneBuilder is prebuild to look for initialize method,
   ...
   public void initialize()
   {
      ...
      myTabPane.getSelectionModel().selectedItemProperty().addListener(new MyTabListener() );
      ...
      */
      ...
      CEPnoteColumn.setCellFactory(TextFieldTableCell.forTableColumn());
       ...
       //Sets the CEPnewButton to do an action event when clicked, utilizes the method newButton();
      CEPnewButton.setOnAction(event -> {
         newButton();
      });
      
      //Uses the adapter to create a new PlayerFileAdapter, and if the adaptor is not equal to null.
      ...
      twPlayer.getSelectionModel().selectedItemProperty().addListener(PlistListner);
      MatchMyListListener = new MatchMyListListener();
      twMatch.getSelectionModel().selectedItemProperty().addListener(MatchMyListListener);
   ...
   /**
    * Updates the tbaleView, but firstly it gets all the players from the binary file.
    * Then it clears the tableView and afterwards inserts the newly updated.
    */
   ...
   /**
    * Takes the given Strings and Integer from the textFields, and places it into the tabeleView columns
    */
   ...
   /**
    * Creates an action event on the remove button, so when it is clicked the action removes the Strings and Integers
    * from the selected row. Afterwards it looks through the PlayerList to checks which Player is removed, and then 
    * saves the changes.
    * @param actionEvent is which actions should be executed
    */
    ...
    * @param playerIntegerCellEditEvent is the given name to initialize the event
    */
    ...
    * @param playerStringCellEditEvent is the given name to initialize the event
    */
   ...
   /**
    * Checks if text fields (name, number, position and note) are empty, and makes sure that if text fields are empty
    * it will pass the given information from the table in their specific text fields 
    * @param e is a short version of an event which is connected to an actionEvent.
    */
   ...
   {
      public void changed(ObservableValue<? extends Match> match, Match oldMatch, Match newMatch)
      {
    ...
    * Button CMaddP Adds player from player from available players list to used player list
    * Were Cup has limit of players for match of 16
    * League has limit of players for match of 15
    * Friendly is only limited by number of available players
    * If number of player has exceeded the allowed number there will be alert
    * If no match type has been selected alert will pop up 
    * @param e
    */
